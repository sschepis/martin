...
import {
  ...
  SHOTSTACK_KNOWLEDGE_FULL, SHOTSTACK_KNOWLEDGE_COMPACT
} from './knowledge/index.ts';
import { SPM_JSON_SCHEMA, SHOTSTACK_JSON_SCHEMA } from './prompts/schemas.ts';
...
`;
  ...
  private async callLLM(systemPrompt: string, userMessage: string): Promise<string | undefined> {
  ...
  async analyzeScript(script: string, options?: any): Promise<ProductionManifest> {
    ...
    const content = await this.callLLM(
      systemPrompt,
      `Analyze the following script and generate a production manifest:\n\n${script}`
    );
  ...
  async generateShotstackComposition(manifest: ProductionManifest, clips: SceneClip[]): Promise<any> {
    const compactness = this.config.promptCompactness || 'full';
...
`;
    ...
    const clipsDescription = clips.map((c, i) =>
      `Clip ${i + 1}: video="${c.videoUrlOrPath}"${c.audioUrlOrPath ? `, audio="${c.audioUrlOrPath}"` : ''}, duration=${c.duration ?? 5}s`
    ).join('\n');
